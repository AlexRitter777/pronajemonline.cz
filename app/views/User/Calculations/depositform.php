...
<div class="main-container">
    ...
    <form method="POST" class="form calculation-form" action="/applications/deposit-calc" name="deposit">

        <?php require_once APP . "/views/Includes/common_calc_data.php"; ?>

        <!--Nájemní smlouva-->
        <div class="form-contract-group">
            <h2 class="calculation-title">IV. Nájemní smlouva</h2>

            <div class="date">
                <label for="contractStartDate" class="label_text">Datum uzavření nájemní smlouvy*</label><br />
                <input type="date" name="contractStartDate" class="field-start-date" id="contractStartDate" value="<?= $data['contractStartDate'] ?? '';?>">
            </div>

            <div class="date">
                <label for="contractFinishDate" class="label_text">Datum ukončení nájemní smlouvy*</label><br />
                <input type="date" name="contractFinishDate" class="field-finish-date" id="contractFinishDate" value="<?= $data['contractFinishDate'] ?? '';?>">
            </div>
        ...
        <div class="form-deposit-items-group">
            ...
            <div class="add_input_fields_deposit_items">
                    ...
                    <?php if(!empty($data['depositItems'][0])) : ?>
                        <?php if(!empty($data['damageDesc'][0])) : ?>
                            ...
                            <div class="description_append">
                                ...
                                <input type="text" name="damageDesc[]" class="description_field" id="damageDesc1" value="<?= $data['damageDesc'][0] ?>" placeholder="Zadejte popis"/>
                            </div>
                            <!-- /.description_append-->
                ...
                <?php if(isset($data['depositItems'])): ?>
                    <?php for ($i = 1; $i < count($data['depositItems']); $i++):?>
                                    <?php if(!empty($data['depositItems'][$i])): ?>
                                        <div class="add_deposit_added_field deposit_added_after" id="<?= ($i+1);?>">
                                            ...
                                            <input type="number" class="right-field" name="depositItemsPrice[]" id="deposit_items_price<?= ($i+1);?>" step="any" placeholder="Zadejte častku" value="<?= ($data['depositItemsPrice'][$i] < 0) ? ($data['depositItemsPrice'][$i] * (-1)) : $data['depositItemsPrice'][$i]  ?>" />
                                        ...
                                        <div class="deposit_append" id="deposit_append<?=  ($i+1);?>">
                                            <?php if(!empty($data['depositItems'][$i])) : ?>
                                                ...
                                                <div class="description_append">
                                                    <label class="label_text">Popis závady/poškození</label>
                                                    <input type="text" name="damageDesc[]" class="description_field" id="damageDesc<?=  ($i+1);?>" value="<?= $data['damageDesc'][$i] ?>" placeholder="Zadejte popis"/>
                                                </div>
                                                <!-- /.description_append-->
                                                <div class="border"></div>
                                            <?php else: ?>
        ...
        <div class="form-deposit-payed-group">
            <h2 class="calculation-title">VI. Složená kauce</h2>

            <div class="zalohy_deposit">
                <div class="zalohy_deposit_label">
                    <label for="deposit" class="label_text">Kauce složená při podpisu smlouvy*</label>
                </div>
                <input type="number" class="field field-ceny-deposit" id="deposit" name="deposit" step="any" placeholder="Zadejte částku" value="<?= $data['deposit'] ?? '';?>" />
            </div>
        </div>
        <!--End Kauce-->
...
<?php require_once APP . "/views/Short_hints/deposit_hints.php"; ?>
